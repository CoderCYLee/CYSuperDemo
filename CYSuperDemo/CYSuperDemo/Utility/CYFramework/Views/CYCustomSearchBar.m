...
#import "CYCustomSearchBar.h"

@interface CYCustomSearchBar () <UITextFieldDelegate>
{
    UIImageView *_iconImgV;
    UIImageView *_iconCenterImgV;
    CYCustomSearchBarIconAlignment _iconAlignTemp;
...
@implementation CYCustomSearchBar
...
- (void)initView {
    ...
    [self addObserver:self forKeyPath:@"frame" options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld context:@"CYFrame"];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if (context == @"CYFrame") {
        ...
        [self adjustIconWith:_iconAlignment];
    } else {
        [super observeValueForKeyPath:keyPath ofObject:object change:change context:context];
    }
}

- (void)setIconAlignment:(CYCustomSearchBarIconAlignment)iconAlignment {
    if (!_iconAlignment) {
        _iconAlignTemp = iconAlignment;
    }
    _iconAlignment = iconAlignment;
    [self adjustIconWith:_iconAlignment];
}

- (void)adjustIconWith:(CYCustomSearchBarIconAlignment)iconAligment {
    if (_iconAlignment == CYCustomSearchBarIconAlignmentCenter && ([self.text isKindOfClass:[NSNull class]] || !self.text || [self.text isEqualToString:@""] || self.text.length == 0) && ![self.textField isFirstResponder]) {
        ...
        if (!_iconCenterImgV)
        {
            _iconCenterImgV = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"123"]];
...
- (void)setPlaceholder:(NSString *)placeholder
{
    ...
    [self setIconAlignment:_iconAlignment];
}

- (void)setBackgroundImage:(UIImage *)backgroundImage
{
    _backgroundImage = backgroundImage;
    _textField.background = backgroundImage;
}
...
- (void)setPlaceholderColor:(UIColor *)placeholderColor {
    ...
    {//TODO: ///asdfasdf
        ...
        {
            _textField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:self.placeholder attributes:@{NSForegroundColorAttributeName: placeholderColor, NSFontAttributeName: _textField.font}];
        }
    }
}
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    if (_iconAlignTemp == CYCustomSearchBarIconAlignmentCenter) {
        self.iconAlignment = CYCustomSearchBarIconAlignmentLeft;
    }
    if (!self.isHiddenCancelButton) {
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    if (_iconAlignTemp == CYCustomSearchBarIconAlignmentCenter)
    {
        self.iconAlignment = CYCustomSearchBarIconAlignmentCenter;
    }
    if (!self.isHiddenCancelButton) {
        [UIView animateWithDuration:0.1 animations:^{
